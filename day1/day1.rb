...
input = File.open("day1_1_input.txt")
...
part_one = modules.map(&:to_i).map{ |mass| fuel_needed_for(mass) }.inject(&:+)
...
part_two = modules.map(&:to_i).map{ |mass| total_fuel_for(mass) }.inject(&:+)
